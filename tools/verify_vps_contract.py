...
def verify(root: Path = ROOT) -> None:
    ...
    require(baseline.get("vcpus") == 2, "baseline.vcpus must be 2", errors)
    require(baseline.get("memory_gib") == 4, "baseline.memory_gib must be 4", errors)
    require(storage.get("kind") == "nvme" and storage.get("gib") == 80, "baseline.storage must be 80 GiB NVMe", errors)
