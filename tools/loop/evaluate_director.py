...
def evaluate(config,cases):
    ...
    for candidate in candidates:
        ...
        for case in cases:
            ...
            created=client.call("POST","/v1/runs",{"input":prompt,"instructions":instructions,"session_id":session},{"Idempotency-Key":session,"X-Hermes-Session-Key":session})
            ...
            results.append({"candidate":candidate["name"],"model_id":candidate["model_id"],"case":case["id"],"prompt_sha256":hashlib.sha256(prompt.encode()).hexdigest(),"run_id":run_id,"usage":result.get("usage"),**verdict})
