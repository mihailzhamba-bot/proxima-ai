...
- **Rule:** `db/migrations/NNN_<snake>.sql`, `BEGIN…COMMIT`, self-checksum `normalized-self-v1`, `tools/verify_migrations.py` (шаблоны политик и грантов - AD-11). main занял `010`; `pa41-full-w2-phase3` при мерже перенумеровывает свою `010` после 016 с пересчётом checksum; `PA-03-02-promotion` списывается как старая версия того же кода. План: `011` `collector_runs`, `collector_run_inputs`, `wb_raw_artifacts`, роль `proxima_run_janitor` + политики, `collector_run_id` в двух существующих таблицах; `012` `stg_wb_orders_obs`, `stg_wb_sales_obs`, view `_latest`; `013` `fact_cabinet_daily`, `fact_cabinet_daily_current`, `data_status_current`; `014` `stg_wb_funnel_obs`, `fact_funnel_daily`, `_current`; `015` `norm_daily`, `norm_daily_current`; `016` `brief_daily`, `brief_current`, роли `proxima_job_collector`, `proxima_job_norm`, `proxima_webapp_readonly`, гранты и политики на базовые таблицы.
...
- **Rule:** каждый job пишет одну JSON-строку на событие (`run_id`, `tenant_id`, `kind`, `step`, без payload) в journald; `collector_runs` - единственный источник статуса прогонов; `OnFailure` → Telegram-канал монитора; dead-man для пользователя - `stale` на `/brief`. Ночной бэкап (`proxima-pg-backup.sh`, age, S3) остаётся; в M-01 добавляется еженедельная проверка восстановления дампа (`make test-db-refresh` в понедельник до `funnel_csv`). `PROXIMA_RAW_DIR` включается в тот же бэкап. Ретеншн: наблюдения, версии и артефакты не удаляются автоматически (Deferred).
...
| Конфигурация и секреты | URI ролей и токены - файлы в `/etc/proxima-ai/secrets/` (`1010:1010 0600`), в контейнеры через compose `secrets:`; значения никогда в env/логах/артефактах; несекретные параметры job - `infra/jobs.env` + флаги CLI + `dim_client_passport` |
| Логи | одна JSON-строка на событие с `run_id`, `tenant_id`, `kind`, `step`; payload только в CAS |
