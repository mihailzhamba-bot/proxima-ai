...
created: '2026-08-30'
updated: '2026-09-09'
revision: 'v3.6 (AD-20: запись решения из webapp, Story 5.0 - предложение, ждёт D-записи Mike)'
...
| Presentation | `/brief` читает view; единственная запись из UI - решение по аномалии (AD-20) | `services/webapp/src/lib/data/postgres-provider.ts` (читатель), `src/app/api/decisions/` + `decision_records` (писатель) |
...
### AD-6 — Расписание снаружи процесса: обязательная утренняя цепочка и фоновая воронка отдельным юнитом

- **Binds:** CAP-1, CAP-5, CAP-6
- **Prevents:** in-process таймеры; независимые таймеры без зависимости от успеха; toolchain на хосте (нет uv/psql/node_modules); юнит без прав на docker; секреты, нечитаемые в контейнере; **фоновая задача в обязательной цепочке** - падение воронки не должно отменять утреннюю сводку; молчащий алерт при единственной неудачной отправке
- **Rule:** `infra/systemd/proxima-morning@.service` + `.timer` (`OnCalendar=*-*-* 05:30:00 Europe/Moscow`, `Persistent=true`; на `.service` - `OnFailure=proxima-alert@%n.service`) запускает `tools/morning_run.sh <tenant>`: обязательная цепочка `collect → norm → brief` строго последовательно, стоп на первой ошибке, каждый шаг - свой прогон. **`funnel_v3` в эту цепочку не входит** (CR к AD-6, решение 4а D25): он фоновый по CAP-6 и уезжает отдельным юнитом `proxima-funnel@.service` с собственным `OnFailure=proxima-alert@%n.service`, запускаемым после успешного `brief`; его падение не отменяет утреннюю сводку и наоборот. Крайний срок 06:30 МСК (FR-22) несёт `TimeoutStartSec` юнита `proxima-morning@`: превышение видно как отказ юнита и уходит в тот же алерт, а не остаётся незамеченным. Отправка алерта не одноразовая: `proxima_alert.sh` вызывает Telegram с `--retry 3 --retry-delay 5`, а проверка пина канала входит в runbook релиза - сторож, о котором никто не узнал, сторожем не является. `proxima-funnel-csv@.timer` - понедельник 06:30 МСК. Имена: `kind` в БД с `_` (`funnel_v3`), юниты с `-` (`proxima-funnel-csv@`), маппинг только здесь. Юниты: `User=root`, `ProtectHome=true`, без `ProtectSystem=strict`/`PrivateDevices` (нужен docker CLI). Шаги = `docker compose --profile jobs run --rm collector|control-plane|control-plane-admin …` из `/srv/proxima-ai/repo`; образы `services/collector/Dockerfile`, `services/control-plane/Dockerfile` (multi-stage, контекст - корень репо, корневой `.dockerignore`: `.env`, `.git`, `node_modules`, `fixtures/`, `_bmad-output/`), процессы в контейнерах `USER 1010` (`proxima-jobs`); все `/etc/proxima-ai/secrets/*_uri` и `<tenant>_wb_*_token` - `1010:1010 0600`, монтируются через compose `secrets:`; `morning_run.sh` передаёт `PROXIMA_GIT_SHA=$(git rev-parse HEAD)` и `PROXIMA_IMAGE_ID=$(docker image inspect -f '{{.Id}}')` в env. `psql`/`pg_dump` - через `docker compose exec postgres`. `proxima-alert@.service` шлёт в Telegram-канал монитора. Бэкфилл идёт по AD-2, а не отдельным правилом: `backfill --source artifact:<sha256_sales>,<sha256_orders> --retrieved-at <ISO>` из артефактов 30.08, импортированных в CAS, затем живой хвост `collect --date-from <run_day-3 артефакта>` с перекрытием; вручную один раз при первом деплое M-01. Ограничения WB те же: `sales` 1/мин, при 80 000 строк в ответе - продолжение с `dateFrom = lastChangeDate` последней строки. [ASSUMPTION] хост-таймеры + контейнерные one-shot вместо `ofelia`/cron в compose - на хосте уже живёт `proxima-host-monitor.timer`.
...
- **Rule:** каждая новая таблица: `tenant_id text NOT NULL REFERENCES tenants`, `ENABLE ROW LEVEL SECURITY`, политика шаблона AD-11; каждый view - `WITH (security_invoker = true)`. TS- и Python-job: `set_config('proxima.tenant_id', $1, false)` первым statement соединения (AD-3); транзакция (3) GUC не переустанавливает. Tenant сентября - существующий `amirova-test`. Единственная схема имён токенов - `/etc/proxima-ai/secrets/<tenant>_wb_<category>_token`; первый релиз M-01 делает `mv` (не symlink) `wb_{statistics,analytics,finance}_token` → `amirova-test_wb_*_token` с chown `1010:1010`; канон analytics-токена - `/etc`, `~/signal-inputs/wb_analytics_token` коллектором не используется; `.env` `WB_*_TOKEN_FILE` остаётся только для `wb_api_probe.py`. `morning_run.sh <tenant>` передаёт пути по шаблону. Второй tenant = строка в `tenants` + три файла + инстанс `proxima-morning@<tenant>` без изменения схемы.
...
- **Rule:** каждый job пишет одну JSON-строку на событие (`run_id`, `tenant_id`, `kind`, `step`, без payload) в journald; `collector_runs` - единственный источник статуса прогонов; `OnFailure` → Telegram-канал монитора; dead-man для пользователя - `stale` на `/brief`. Ночной бэкап (`proxima-pg-backup.sh`, age, S3) остаётся; в M-01 добавляется еженедельная проверка восстановления (`proxima-restore-check@`, понедельник 06:00: последний локальный дамп из `/var/backups/proxima/*.sql.gz` (факт 31.08: plain gzip, age только для S3-копии) - `gunzip | psql proxima_test` после `DROP/CREATE`, затем `SELECT count(*)` по `fact_cabinet_daily_current`; это проверка бэкапа, а `make test-db-refresh` из живой базы - для тестов). `PROXIMA_RAW_DIR` включается в тот же бэкап. Ретеншн: наблюдения, версии и артефакты не удаляются автоматически (Deferred).
...
### AD-20 — Запись из webapp: append-only `decision_records`, отдельная роль только на INSERT, «прогон» решения = сама строка

- **Binds:** CAP-5, CAP-8
- **Prevents:** синтетический прогон `kind = 'decision'` в `collector_runs` (CHECK из 011 под additive-only неизменяем - единица упёрлась бы в гейт после того, как написала код); `run_id` решения, ссылающийся на прогон `brief` (CASCADE стирает решение при откате, RESTRICT блокирует откат); `orphaned` как колонка, которую кто-то `UPDATE`-ит (мутация append-only строки; писателю пришлось бы дать UPDATE); исход сверки, записанный правкой строки решения (история решений исчезает, SM-9 неисчислим); одно «текущее» на двух писателей (исход прячет решение, экран показывает не то, что человек отметил); немонотонное «открыто» (новая отметка после сверки возвращает запись в открытые и выбрасывает исход); payload с полями, которых нет в `decision-record` v1 (`additionalProperties: false` рубит INSERT), и строка-исход, не проходящая `required`; Ajv в webapp (его там нет, новые зависимости запрещены) и схема, которой нет в образе; политика `FOR INSERT … USING (…)` (PostgreSQL принимает у `FOR INSERT` только `WITH CHECK`, а `verify_migrations.py` требует `USING` - две единицы напишут две разные несобирающиеся миграции); одноаргументный `current_setting('proxima.tenant_id')` из текста истории; гранты без политик (роль с грантом, но без политики видит 0 строк); роль, которой bootstrap никогда не выдаст членство (жёсткий список `IN (…)` в скрипте); гейт, которому неоткуда взять DSN; `INSERT … RETURNING` под ролью без SELECT; запись и чтение витрины под одной LOGIN-ролью (бюджет SELECT AD-9 и least privilege); писатель, доступный из серверного компонента через барель `src/lib/data/index.ts`; запись из режима `fixtures`, где читатель отдаёт демо-данные; URI писателя с владельцем `1010:1010` (образ webapp - uid 1001); ответ экрана до `COMMIT`; write-эндпоинт, открытый наружу до октябрьской auth; новая колонка-состояние с CHECK-перечислением (повтор ловушки 011)
- **Rule:** запись решения по сигналу (Story 5.3) идёт в `decision_records(record_id uuid PRIMARY KEY, tenant_id text NOT NULL REFERENCES tenants(tenant_id) ON DELETE RESTRICT, signal_id text NOT NULL CHECK (signal_id ~ '^[a-z0-9][a-z0-9-]{1,127}$'), brief_day date NOT NULL, source_run_id uuid NOT NULL, run_id uuid NULL REFERENCES collector_runs(run_id) ON DELETE CASCADE, actor text NOT NULL, recorded_at timestamptz NOT NULL DEFAULT clock_timestamp(), git_sha text, image_id text, payload jsonb NOT NULL)`; зерно - **одна строка на запись**, логический ключ `(tenant_id, signal_id)`, индексы `(tenant_id, signal_id, recorded_at DESC)`, `(run_id)`, `(tenant_id, brief_day)`. Таблица только для решений по сигналам: ручные вводы CM-19/CM-1 получают свою таблицу по этому же образцу, ту же группу и тот же пул (Deferred) - их у сигнала нет, а `signal_id`/`brief_day`/`source_run_id` здесь `NOT NULL`. Таблица append-only: `UPDATE` не выдаётся ни одной роли; `DELETE` janitor получает автоматически (bootstrap выдаёт его каждой таблице с колонкой `run_id`), поэтому «строку человека не удаляют» - конвенция плюс RLS-тест, как AD-11 уже делает для `UPDATE collector_runs`, а не привилегия. Два писателя, один разделитель: `run_id IS NULL` ⇔ строку написал человек из UI (она сама себе прогон: `record_id` - её идентификатор, `git_sha`/`image_id` - сборка образа webapp, nullable как в `collector_runs`); `run_id IS NOT NULL` ⇔ строку написал прогон `brief` утренней цепочки (исход сверки) и она обычная версия по AD-3, исчезающая по CASCADE при откате этого прогона. Синтетического прогона `kind = 'decision'` **не существует**: CHECK `collector_runs.kind` из 011 под AD-14 заморожен (`DROP CONSTRAINT` запрещён, `ADD CONSTRAINT` только сужает), а у записи из одной строки нет фазы `RUNNING` и частичного состояния - открывать и закрывать нечего. Первый consequence FR-16 («запись обратима по `run_id`») и Given Story 5.3 («записи помечаются `orphaned`, не удаляются») сужаются: обратимы исходы сверки, решения человека - нет. `decision_records_current WITH (security_invoker = true)` - **одна строка на сигнал и один SELECT на поверхность**: последняя строка решения (`DISTINCT ON (tenant_id, signal_id) … WHERE run_id IS NULL ORDER BY recorded_at DESC, record_id DESC`), `LEFT JOIN collector_runs` по `source_run_id` даёт `(r.run_id IS NULL) AS orphaned`, второй `LEFT JOIN` подклеивает последнюю строку-исход того же сигнала **строго новее решения** (`o.recorded_at > d.recorded_at`) и `status` = `open`, когда её нет. Состояние монотонно: новая отметка человека после сверки снова открывает запись и исход к ней не липнет; счётчики SM-4/SM-9 считаются по истории таблицы, а не по `_current`. `orphaned` осмыслен только под ролью с грантом и политикой на `collector_runs` (`proxima_webapp_readonly` их имеет по 011) - db-тест бежит под ней, не под `proxima_sandbox` с BYPASSRLS. `source_run_id` - прогон `brief`, чью строку `brief_current` человек видел; **FK на него не ставится намеренно**: с `ON DELETE CASCADE` откат стирал бы решение, с `RESTRICT` - блокировал бы `delete_run.py`, с `SET NULL` - мутировал бы append-only строку и терял провенанс. Откат прогона фактов сам делает решения по его сигналам `orphaned = true`, сохраняя текст, автора и дату; `tools/delete_run.py` не меняется и таблицы не знает (три явных DELETE, остальное - CASCADE, а проверки ссылочной целостности RLS обходят по построению), но его `--dry-run` печатает только `runs`/`artifacts`/`run_inputs` - сколько строк-исходов уйдёт по CASCADE, план не показывает (счётчик CASCADE-таблиц - Deferred). `signal_id` детерминирован (`scn001-<день>-<уровень>-<ключ>`, `detector/signals.py`) и прогон не кодирует, поэтому провенанс несут именно колонки `source_run_id` и `brief_day`. `payload` валиден по `contracts/decision-record.schema.json` v1 (AD-10), схема не расширяется, а обе формы записаны точно: строка решения - полный документ с `actual: null`, `delta: null` и **без** ключа `outcome` (схема его при `actual = null` запрещает), `horizon_days` живёт внутри `expected`, а не корневым полем; строка-исход - тот же документ с заполненными `actual`, `delta`, `outcome`, а `decision`, `reason`, `expected`, `actor`, `decided_at` копируются из `decision_records_current` дословно (`actor` исхода - автор решения, а не роль джоба: строку пишет прогон, авторство остаётся человеческим). Валидация двусторонняя и без новых зависимостей: в webapp - структурная проверка по самой схеме (образец `src/tests/brief-fixtures.test.ts`; Ajv в webapp нет и новые зависимости запрещены, а `contracts/*.json` в образ не копируются), в control-plane - полная `jsonschema`-валидация против файла из `contracts/` (путь AD-10), примеры - гейт `make contracts`. Колонки `signal_id`, `actor`, `recorded_at` дублируют payload как ключи для индексов и RLS - так же, как `brief_daily.status` дублирует статус внутри payload. Роли по шаблону AD-11, миграция `019_decision_records.sql` (**целевой** номер по AD-14: вторая мержащаяся ветка перенумеровывает с пересчётом checksum; additive-only, `BEGIN…COMMIT`, self-checksum `normalized-self-v1`, таблица + view + индексы + гранты + RLS + политики в одном файле): NOLOGIN-роль `proxima_ui_writer` (`CREATE ROLE proxima_ui_writer NOLOGIN` - единственная форма, которую пропускает `verify_migrations.py`; набор ролей открыт - новая заводится в миграции своей первой таблицы, а не только в 011) получает **только** `GRANT INSERT ON decision_records`, без SELECT/UPDATE/DELETE и без гранта на view; `proxima_webapp_readonly` - `GRANT SELECT` на таблицу и view; `proxima_job_norm` - `GRANT SELECT, INSERT`; `proxima_run_janitor` - DELETE вне ledger. Политик **четыре**, гранта без политики не бывает: `FOR ALL TO proxima_ui_writer USING (tenant_id = current_setting('proxima.tenant_id', true)) WITH CHECK (…)` («только INSERT» задаёт грант, а не форма политики: `FOR INSERT` в PostgreSQL принимает только `WITH CHECK`, гейт требует `USING`, и `USING` без SELECT/UPDATE/DELETE-гранта недостижим), `FOR SELECT TO proxima_webapp_readonly`, `FOR ALL TO proxima_job_norm`, `FOR ALL TO proxima_run_janitor`; двухаргументный `current_setting(…, true)` обязателен везде. LOGIN-пользователь - вне ledger, в `infra/bootstrap/provision-runtime-roles.sh`: `proxima_webapp_writer` LOGIN NOSUPERUSER NOBYPASSRLS INHERIT, член `proxima_ui_writer`. Скрипт ведёт роли **ручными списками**, и новая роль дописывается в каждый из них, иначе провал будет тихим: пароль (`read_or_create_password`), верификатор и цикл проверки `SCRAM_RE`, `CREATE ROLE`/`ALTER ROLE`, `GRANT CONNECT` + `USAGE ON SCHEMA public` на `proxima`, `add_membership` **и** жёсткий `IN (…)` запроса `EXISTING_GROUPS` (без него членство не выдаётся никогда, скрипт печатает `ok`, а `INSERT` падает `permission denied`), `write_uri`, явная строка `chown 1001:1001` (job-глобы `proxima_{collector,norm,janitor,sandbox}_*` новый файл не задевают, а глоб `proxima_webapp_*` запрещён тестом), `GRANT CONNECT` в `TEST_SQL` для `proxima_test`, и строка-отчёт в конце (её текст закреплён `tools/tests/test_provision_runtime_roles.py`). URI - `/etc/proxima-ai/secrets/proxima_webapp_writer_uri` (`1001:1001 0600`, uid образа webapp - исключение D35, как у `proxima_webapp_uri`), в контейнере `/run/secrets/proxima_webapp_writer_uri` через `secrets:` **staging-overlay `infra/webapp.staging.compose.yaml`** (сервиса `webapp` в `infra/compose.yaml` нет), env `WEBAPP_WRITE_DATABASE_URI_FILE` - путь внутри контейнера; там же `PROXIMA_GIT_SHA`/`PROXIMA_IMAGE_ID` в `environment:` по форме сервисов `collector`/`control-plane`, и закрытый список env витрины в runbook 2.6 (`docs/operations/release-m03.md`) пополняется теми же двумя строками. Имя `proxima_webapp_writer` остаётся у LOGIN-пользователя, как просит история; группа названа `proxima_ui_writer`, потому что в PostgreSQL имена ролей общие, а ручные вводы вступят в ту же группу. Витрина держит **два пула, не один**: читатель `postgres-provider.ts` под `proxima_webapp` не меняется; писатель - `services/webapp/src/app/api/decisions/writer.ts` (не `src/lib/data/`: барель `src/lib/data/index.ts` реэкспортировал бы его в любой серверный компонент, и текстовый гейт этого не увидел бы) со своим `Pool` (`max: 2`, ленивый), своим URI-файлом и тем же `set_config('proxima.tenant_id', $1, false)` первым statement соединения (AD-13), используемый только из `src/app/api/decisions/route.ts` (POST; `src/app/api/auth/` заморожен AD-18 и не трогается). Решения на экране читает читатель отдельным бюджетом ≤ 1 SELECT из `decision_records_current`; в `brief_daily.payload` исходы не переносятся. Вне `WEBAPP_DATA_MODE=postgres` формы решения нет, а POST отвечает 403: писать решение по фикстурному сигналу нельзя. `actor` до октябрьской auth - env `WEBAPP_ACTOR`, валидируется при старте процесса как `WEBAPP_TENANT_ID` (пустое или незаданное значение роняет старт, а не кладёт пустую строку). Атомарность «запись до закрытия экрана»: handler делает один `INSERT` в транзакции и отвечает 200 только после `COMMIT`, оптимистичного UI нет; `record_id` генерирует handler и он же ключ идемпотентности формы - `INSERT … ON CONFLICT (record_id) DO NOTHING` (требует только INSERT-гранта), поэтому повтор POST после таймаута не превращается в «человек передумал»; `INSERT … RETURNING` не используется - он требует SELECT-гранта, которого у писателя нет. `make verify` получает таргет `decision-records` (гейт-строка `decision-records: writer INSERT only, orphaned survives delete_run`): (1) `tools/verify_webapp_write_boundary.py` по образцу `verify_wb_client.py` - без PG, проверкой путей: `WEBAPP_WRITE_DATABASE_URI_FILE` упомянут ровно в одном модуле, писатель лежит под `src/app/api/decisions/` и не реэкспортируется из `src/lib/data/index.ts`, в `postgres-provider.ts` нет пишущего SQL; (2) `tools/tests/test_provision_runtime_roles.py` - новая LOGIN-роль, её URI-файл, владелец `1001:1001` и обновлённая строка-отчёт; (3) `services/collector/tests/decision-records.db.test.ts` (носитель `*.db.test.ts` - workspace коллектора, AD-12) под DSN `PROXIMA_TEST_DSN_WEBAPP_WRITER`, который `tools/pg_local_roundtrip.sh` экспортирует по тому же правилу «LOGIN-пользователь → суффикс», что и пять существующих: строки RLS-матрицы (`INSERT` с GUC проходит, `INSERT` с чужим `tenant_id` падает на `WITH CHECK`, `SELECT`/`UPDATE`/`DELETE` - `permission denied`; под `PROXIMA_TEST_DSN_WEBAPP` view отдаёт `> 0` с GUC и `0` без, `INSERT` - `permission denied`) и сценарий `orphaned after delete_run` (решение по сигналу прогона `brief` → `tools/delete_run.py` этого прогона → строка решения на месте, текст, автор и дата не изменились, `orphaned = true`, строка-исход того же прогона исчезла). Пункты (1)-(2) выполняются везде, (3) - только там, где есть PG16 (CI ставит `postgresql-16`; в сандбоксе OpenHands `pg-roundtrip` = `SKIP`, как у `delete-run` и `nm-daily`). Общий урок из 011, обязательный для новых таблиц: CHECK-список, который заведомо будет расти, под additive-only замораживается навсегда - такие поля задаются формой (`CHECK (col ~ '^[a-z][a-z0-9_]*$')`), а не перечислением; `decision_records` поэтому не заводит колонку-состояние, единственный разделитель писателей - `run_id IS NULL`, а рост значений `decision` = `decision-record` v2 по AD-10. Принятие AD-20 требует трёх однострочных правок в соседних AD, и без них спайн противоречив: AD-3 («все новые таблицы: `run_id NOT NULL … CASCADE`») получает исключение для `decision_records`; AD-9 («ровно два `SELECT`») переходит на бюджет **на поверхность** (сводка - два, полоса метрик - два, решения - один), как уже уточнил D36-C3; AD-16 («разрешённые рёбра только как на диаграмме») получает ребро webapp → Postgres на запись. [ASSUMPTION] write-эндпоинт живёт только внутри конверта AD-15 (loopback `:3000` + ssh-туннель `proxima-app`); переход на Caddy/домен без auth его открывать не должен - блокирующее условие октябрьского релиза, автоматического гейта на это нет.

## Consistency Conventions

| Concern | Convention |
| --- | --- |
| Naming | таблицы `stg_wb_<dataset>_obs`, `fact_<grain>_daily`, справочники `dim_<entity>` (версии по `run_id`, как факты), view `<table>_latest` / `<table>_current`; `kind` с `_`, юниты `proxima-<job>@<tenant>` с `-`; схемы `contracts/<entity>.schema.json`; NOLOGIN-группы `proxima_job_*`, `proxima_webapp_*`, `proxima_run_*`, `proxima_ui_*`, LOGIN-пользователи - имя процесса (`proxima_webapp`, `proxima_webapp_writer`); идентификаторы английские, документация русская |
...
| Записи | наблюдения append-only, `DO NOTHING` только при том же `canonical_sha256`; факты - версии по `run_id`, читаются через `_current`; `UPDATE` только `status`/`finished_at` в `collector_runs`; `decision_records` append-only, `UPDATE` не выдаётся ни одной роли (AD-20) |
...
erDiagram
  ...
  collector_runs ||--o{ decision_records : outcome_rows_cascade
  brief_daily ||..o{ decision_records : source_run_id_soft_link
...
db/migrations/        # целевые: 011_run_ledger, 012_stg_wb_orders_sales, 013_fact_cabinet_daily; E2: norm_daily, brief_daily; E3: funnel; E4: 018_nm_daily (AD-19); E5: 019_decision_records (AD-20) - вторая мержащаяся ветка перенумеровывает
infra/
...
services/webapp/src/lib/data/postgres-provider.ts   # читатель, ровно два SELECT на поверхность (AD-9)
services/webapp/src/app/api/decisions/            # AD-20: route.ts (POST, ответ после COMMIT) + writer.ts (свой Pool и URI, только INSERT)
services/webapp/src/lib/contracts/                  # codegen
tools/
  delete_run.py  morning_run.sh  record_fixture.ts  anonymize_fixture.py  cas_import.ts  verify_wb_client.py  test_db_refresh.sh  verify_webapp_write_boundary.py
```
...
| CAP-8 план действий (M-05) | `diagnosis/` (mock LLM) → `signals[].diagnosis`; решение человека - `decision_records` + `decision_records_current`, писатель `webapp/src/app/api/decisions/writer.ts`, исход сверки пишет прогон `brief` | AD-10, AD-20 (предложение, ждёт D-записи Mike); провайдер LLM - Deferred |
...
- Расширение `collector_runs.kind`: CHECK из 011 под additive-only не расширяется (`DROP CONSTRAINT` запрещён, `ADD CONSTRAINT` только сужает) и остаётся замороженным. AD-19 обошёл это, разместив детектор шагом `brief`; AD-20 - разделителем `run_id IS NULL` вместо вида прогона `decision`. Открытым остаётся теневой пересчёт Epic 6: своего вида прогона у него нет, и решение (исключение в AD-14 для CHECK-списков, отдельный реестр или файлы `verification/golden/` без строки в реестре) принимается в единице 6.1/6.2.
- Второй писатель в `proxima_ui_writer` - ручные вводы плана поставок и переопределений паспорта (CM-19, CM-1, OQ-8): своя таблица по образцу AD-20, тот же LOGIN и тот же пул; отдельный реестр «прогонов человека» заводится не раньше третьего случая (правило трёх).
- `actor` из сессии вместо env `WEBAPP_ACTOR` и защита write-эндпоинта - вместе с включением auth в октябре (AD-18); до этого запись живёт только за loopback + ssh-туннелем.
- Счётчик строк CASCADE-таблиц в `tools/delete_run.py --dry-run`: сегодня план печатает только `runs`/`artifacts`/`run_inputs`, и оператор не видит, сколько строк-исходов (AD-20) и версий фактов уйдёт вместе с прогоном.
- Перенос исходов сверки в `brief_daily.payload` (чтобы решения читались тем же SELECT, что сводка) - не раньше, чем счётчики SM-4/SM-9 покажут, что отдельного бюджета чтения мало.
