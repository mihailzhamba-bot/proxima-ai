...
**Given** Stories 1.0-1.7, 1.9, 1.10, 1.12, 1.13 смержены (1.8 - октябрь, 1.11 - в составе 2.5: правило сжатия), в том числе 1.5 с реальным бэкфиллом (без синтетических строк); теневой пересчёт шагов 1-4 и 6 (Story 6.1) выполнен - расхождений нет либо каждое объяснено записью в `docs/state/SHADOW-RECONCILIATION.md` (CP-12); тег `v2026.09.NN-2`, runbook по CP-1, явное «деплой» от Mike (D7); дата деплоя стоит в календаре Mike - **вт 15.09.2026** (D33; не позже 20.09 при выполненном бэкфилле)
