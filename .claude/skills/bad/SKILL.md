...
## Startup Gate: the conductor must be stopped (PROXIMA AI local override)

**Before anything else**, spawn a `MODEL_STANDARD` subagent to run:

```bash
systemctl is-active codex-conductor.timer; conductor status 2>/dev/null | head -c 400
```

If the timer is `active`, or `conductor status` reports any entry in `workers`,
**HALT immediately** and tell the user:

> Дирижёр (`codex-conductor.timer`) активен и ведёт очередь тех же историй.
> Два оркестратора над одной `sprint-status.yaml` подерутся за истории и ветки.
> Останови его командой `sudo systemctl disable --now codex-conductor.timer`,
> дождись, пока текущие воркеры закончат, и запусти `/bad` снова.

Do not start Phase 0, do not create worktrees, do not touch sprint-status.yaml.
`tools/orchestrator/bad_dev_story.sh` enforces the same gate itself and exits 3,
so a missed instruction here still cannot dispatch work — but stopping at the top
avoids half-built worktrees.
...
### Step 3: Develop Story — delegated to OpenHands (`MODEL_STANDARD`)

> **LOCAL OVERRIDE (PROXIMA AI).** Upstream BAD implements this step with a local
> subagent running `/bmad-dev-story`. Here the implementation is executed by an
> OpenHands worker on this VPS through `tools/orchestrator/bad_dev_story.sh`.
> Everything else in this skill is unmodified. See `DECISIONS.md` (BAD + OpenHands)
> and `docs/agent-system/ORCHESTRATOR.md` for the dispatch-prompt contract.

Spawn with model `MODEL_STANDARD` (yolo mode):
```
You are the Step 3 dispatcher for story {number}-{short_description}.
Working directory: {repo_root}/{WORKTREE_BASE_PATH}/story-{number}-{short_description}.
Auto-approve all tool calls (yolo mode). You do NOT write the implementation
yourself — an OpenHands worker does, and you carry its commits back.

1. Write the dispatch prompt to
     {repo_root}/logs/openhands-bridge/story-{number}/dispatch.md
   ...
   template in `docs/agent-system/ORCHESTRATOR.md` (section «Шаблон dispatch-промта»):
     ...
     - hard rules: do not touch db/migrations/* except a new one, tools/verify_*.py,
       .github/workflows/*, the webapp auth zone, services/control-plane/src/proxima/;
       no network in tests; `git add` only your own files; bash-3.2-compatible scripts;
       "if a fact is not in the documents, ask in the report — do not invent it"
     - gate: `PUPPETEER_SKIP_DOWNLOAD=1 make verify` fully green
     - do NOT push and do NOT open a PR
     - final report format: commits (hash + subject), verify tail with PASS lines,
       file list, open questions

2. Run the bridge from the REPO ROOT (not the worktree):
     tools/orchestrator/bad_dev_story.sh \
       --run-id story-{number} \
       --branch story-{number}-{short_description} \
       --base-ref <HEAD of the story worktree, i.e. Step 2's last commit> \
       --source-dir {repo_root}/{WORKTREE_BASE_PATH}/story-{number}-{short_description} \
       --prompt-file {repo_root}/logs/openhands-bridge/story-{number}/dispatch.md
   It prints ONE JSON object. Read `exit_code`, `status`, `commits`, `local_ref`
...
4. On any non-zero exit_code — STOP this story and report. Do not retry, do not
   implement it yourself, do not auto-confirm anything. The codes mean:
     ...
     zero commits) · 5 timeout, conversation left running · 6 infrastructure
     · 7 integrity (wrong base, ancestry, protected path, secret in the diff)

Report: the JSON object verbatim, plus success or failure with error details.
```
