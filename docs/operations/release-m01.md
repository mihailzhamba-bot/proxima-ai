...
Обновлено 08.09.2026: сверен с `origin/main` (`f45243e`, миграции 001-018) и с боевым сервером только чтением (`docs/state/RELEASE-READINESS-1.14.md`, §5 и блокер B3); что и почему изменилось - раздел «Сверка 08.09.2026» внизу. Исполняет Claude - сессия оркестратора на VPS `proxima` (135.106.186.210) под `proxima-admin` - после явного «деплой» от Mike в чате (D7: «Деплой - Claude, после явного «деплой» в чате на каждый релиз»); за Mike остаются решения раздела 1 (переименование токенов, правка `.env`), если он предпочитает сделать их сам. Дата релиза - вт 22.09.2026 (перенос с 15.09 подтверждён Mike 20.09.2026 в чате; исходная дата - D33, PR #89); слово «деплой» в чате в день релиза (D7). Каждая команда - копипаста, под каждой сказано, что должно появиться в ответ. Если ответ другой - не импровизировать, а идти в раздел 7 «Откат».
...
```
Релизный тег - `v2026.09.NN-2` по AC Story 1.14, `NN` = день деплоя: деплой вт 22.09.2026 (подтверждено Mike 20.09.2026 в чате), то есть `v2026.09.22-2`. Ставится на `main` за сутки до релиза и записывается здесь и в `CHANGELOG.md` (раздел 8). На 08.09 тега ещё нет (`git ls-remote --tags origin` → пусто). Ожидается коммит тега и `018_nm_daily.sql` - последняя миграция в `main` на 08.09 (Story 4.0, PR #95). Если в теге файлов больше, номер последней миграции - ожидание раздела 2.
...
Токены переименовываются под схему `<tenant>_wb_<category>_token` и отдаются пользователю контейнеров `1010`. Uid 1010 - номинальный пользователь образов (`services/collector/Dockerfile`: `useradd --uid 1010 proxima-jobs` внутри образа); на хосте записи в `/etc/passwd` нет (`getent passwd 1010` → пусто), и это нормально: `chown 1010:1010` работает по числу, а compose биндит secret-файлы «как есть» (`infra/compose.yaml`: «Secret-файлы на хосте - 1010:1010 0600 (AD-6)»). Единственное исключение - два файла роли webapp, `proxima_webapp_password` и `proxima_webapp_uri`: образ webapp работает под uid 1001 (`services/webapp/Dockerfile`, оставлен намеренно - у публичного процесса свой uid), и provision в 1.4 отдаёт их `1001:1001` (решение Mike 08.09, addendum к D35; находка репетиции). Токены и owner-секреты из этого шага - по-прежнему `1010`.
...
```bash
sudo stat -c '%A %u:%g %n' /etc/proxima-ai/secrets/proxima_webapp_uri /etc/proxima-ai/secrets/proxima_webapp_password /etc/proxima-ai/secrets/proxima_collector_uri
```
Ожидается `-rw------- 1001:1001` для двух файлов webapp и `-rw------- 1010:1010` для `proxima_collector_uri` (в `sudo ls -l /etc/proxima-ai/secrets/proxima_webapp_uri` - `1001 1001`; если на хосте есть пользователь с uid 1001, `ls` покажет его имя, `stat -c %u` - всегда число). Другой владелец - повторить прогон скрипта: владельцев он переустанавливает на каждом запуске.
...
**Ожидается по репетиции наката 09.09** (одноразовый стенд `proxima-migtest`: ночной дамп боевой базы, схема 6, поверх него те же двенадцать миграций; протокол - `docs/state/API-FACTS.md`, раздел «Репетиция наката миграций 007-018 поверх боевого дампа»): `18|18`; 48 объектов в схеме `public` (35 таблиц + 13 вьюх); 58 политик RLS на 24 таблицах; строки пилота не меняются ни в одной из тринадцати старых таблиц (`stg_wb_nm_report_rows` 1220, `dim_warehouse_map` 60, `business_signal_raw_artifacts` 27, `business_signal_runs` 11, `dim_product` 9, `raw_wb_analytics_responses` 7, `tenants` 2 - числа на 09.09, к дню релиза вырастут, важно равенство до и после); новые таблицы лестницы созданы и пусты. Проверить можно теми же запросами:

```bash
printf "SELECT count(*) FROM information_schema.tables WHERE table_schema='public';\n" | sudo proxima-psql-readonly
printf "SELECT count(*) FROM pg_policies WHERE schemaname='public';\n" | sudo proxima-psql-readonly
```

Ещё одно ожидаемое следствие: боевая база несёт `ALTER DEFAULT PRIVILEGES FOR ROLE proxima GRANT SELECT ON TABLES/SEQUENCES TO proxima_diagnostics` (видно в `pg_default_acl`), поэтому каждая созданная миграциями таблица и вьюха бесшумно получит `SELECT` для роли `proxima_diagnostics` - на репетиции это 48 «лишних» грантов против базы, собранной с нуля. Это норма, а не дефект наката: роль `NOLOGIN`, `NOBYPASSRLS`, ни одна политика её не называет, и под `SET ROLE` она читает 0 строк из всех таблиц с RLS. Ничего снимать и чинить не нужно; знать нужно, чтобы аудит грантов после релиза не принял это за расхождение.
...
Зачем: код коллектора и control-plane из `main` ни разу не прогонялся end-to-end на артефактах 31.08, а боевой контур (схема 6, таймеров нет) до релиза не трогается. По D35 (Mike, 08.09) разделы 2-4 этого runbook прогоняются на этом же VPS в **одноразовом compose-проекте** `proxima-rehearsal`; единственное исключение - живой хвост: 2 read-вызова WB (`supplier/orders`, `supplier/sales`) на statistics-токене. Слово «деплой» для репетиции не требуется, но `tail` без явного `--live` не запускается. Инструменты: `infra/compose.rehearsal.yaml` (override к `infra/compose.yaml`) и `tools/rehearsal_run.sh`; оба в `main` после мержа ветки `feat/rehearsal-stack`, боевой `compose.yaml` не меняются.
...
| Секреты / raw | `/etc/proxima-ai/secrets`, `/srv/proxima-ai/raw` | `<root>/secrets`, `<root>/raw` (`1010:1010`, `0700`/`0600`; `proxima_webapp_*` - `1001:1001`, uid образа webapp) |
...
**Что отличается от релиза 22.09** (ожидания скрипта под это подстроены):
...
```
`init` делает `<root>/{secrets,raw,bin,logs,.env}`; `ls -la <root>/secrets` в конце показывает 14 файлов `0600` (`postgres_user`, `postgres_password`, по `<роль>_password` и `<роль>_uri` для пяти ролей, два токена): 12 - `1010:1010`, а `proxima_webapp_password` и `proxima_webapp_uri` `init` пишет для uid 1001 (`1001:1001` - пользователь образа webapp, `services/webapp/Dockerfile`; то же правило в §1.4). Имя токена-источника - как на сервере на 08.09 (`wb_statistics_token`); после раздела 1.2 файл называется `amirova-test_wb_statistics_token`. Повторный `init` на существующем `<root>/.env` отказывает.
...
22. **Накат 007-018 поверх боевой схемы 6 прорепетирован (M1, D37, 09.09 10:15 UTC).** Последний непроверенный шаг §6 п. 4 готовности 1.14: и репетиция D35, и CI `apply-migrations-in-container`, и `pg-roundtrip` стартуют с пустого тома, а на сервере миграции лягут поверх тринадцати таблиц с данными пилота. Одноразовый compose-проект `proxima-migtest` (свой postgres `127.0.0.1:5435`, своя сеть и том, override снимает initdb-монтирование `db/migrations`) поднят пустым, в него восстановлен ночной дамп `/var/backups/proxima/2026-09-09-proxima.sql.gz`, затем прогнан `apply-migrations` через `control-plane-admin` - exit 0, 007…018 одним проходом. Результат: `18|18`, 48 объектов, 58 политик, все строки пилота на месте, новые таблицы лестницы пусты, `provision-runtime-roles.sh` идемпотентен (второй проход - только пять `NOTICE … already been granted membership`). Сверка со стендом из нуля: колонки 429 = 429, индексы 86 = 86, политики 58 = 58, расхождений ноль; единственная разница - 48 грантов `SELECT` роли `proxima_diagnostics` из `pg_default_acl` боевой базы (§2, «Ожидается по репетиции наката 09.09»). Отдельно для §7: `pg_dump` одной базы несёт GRANT'ы, но не роли - restore боевого дампа в чистый кластер падает на `ERROR: role "proxima_diagnostics" does not exist`, роли-грантополучатели надо создать заранее (`NOLOGIN`-заглушки достаточно). Боевой `proxima-ai-postgres-1` не трогался; протокол и все числа - `docs/state/API-FACTS.md`.
