...
|---|---|---|
| GitHub `mihailzhamba-bot/proxima-ai` | приглашение в репозиторий с правом Write на существующий аккаунт человека; 2FA до принятия приглашения | ветки и PR по `CONTRIBUTING.md`; `main` защищён ruleset `main-protected` (D34): PR, одобрение владельца, зелёные проверки; мерж делает владелец |
