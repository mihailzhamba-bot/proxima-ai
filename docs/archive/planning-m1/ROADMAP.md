...
6. LLM runtime, Ozon, WB Advertising APIs, любые WB WRITE, client-facing UI и Torgstat live session automation не входят в M1.[^dec006]

[^dec006]: DEC-006 (2026-08-17, Jira PMM-30, `docs/agent-system/DECISIONS.md`): вне M1-контура LLM runtime и client-facing UI разрешены отдельным решением при трёх условиях одновременно - только staging-данные пилота, обязательная маркировка доверия `unreleased` на каждом выводе, production release pointer не двигается. Формулировка M1-границы в этом пункте не меняется. WB WRITE, Ozon, WB Advertising API и Torgstat live session automation остаются запрещены без изменений (DEC-005).
