...
PA-49 (web-кабинет v0, подзадача 1/5 PA-38), сессия 1 завершена 2026-08-25, коммит `93f0db1` на локальном `main` (НЕ запушен; запланированная ветка `feat/pa-49-webapp-skeleton` была переключена на main параллельной сессией посреди работы — reflog подтверждает, содержимое коммита цело; урок: один write-воркер на дерево, см. AGENTS.md). Каркас: `services/webapp` (Next.js 16.3 standalone + Better Auth 1.7 lazy + Tailwind 4 + Drizzle, две темы, GYR-токены, ₽-форматтеры, шелл с unreleased-плашкой DEC-006, скелеты brief/inbox/dashboard/admin + styleguide, vitest 11 тестов), деплой-контур (Dockerfile, `infra/webapp.compose.yaml` overlay + Caddyfile; основной compose.yaml не тронут), Makefile-хуки, `.gitignore` + `**/.next/` + `*.tsbuildinfo`. `make verify` PASS целиком; smoke standalone-сервера зелёный. Living plan: `docs/exec-plans/active/pa-49-webapp-skeleton.md`. Сессия 2 = деплой на VPS: нужны от Mike домен + A-запись app.<домен> → 135.106.186.210 и подтверждение открытия 80/443; на VPS создать схему webapp_auth + роли webapp_auth_writer/webapp_readonly (additive, вне M1-ледаря), собрать образ, поднять overlay compose, завести первого пользователя, заменить cookie-presence на auth.api.getSession. Затем PA-50 (живой Daily Brief на fixtures; контракт сигнала брать у PMM-29 bot lane). Jira: PA-49 In Progress; PA-50..54 созданы со связями; PA-55 (DEC-запись про публичный URL) To Do. Локальный превью: `npm --workspace @proxima/webapp run dev` → http://localhost:3000.
...
- PA-49 сессия 1 (this agent, ветка `feat/pa-49-webapp-skeleton`): новый `services/webapp/**`, `infra/webapp.compose.yaml`, `infra/Caddyfile`; изменены `.gitignore`, `Makefile`, корневой `package.json`, `package-lock.json` (регенерён npm install — принадлежит этой задаче), `docs/agent-system/TASKS.md` + этот файл, `docs/exec-plans/active/pa-49-webapp-skeleton.md`. Чужие непрокоммиченные нити в дереве: untracked `.autopilot/`, `proxima-project-map-2026-08-22.html` — не стейджить.
...
- eslint-config-next 16 экспортирует flat-конфиги напрямую (без FlatCompat); eslint пин 9.39.5 — eslint-plugin-react из next-конфига падает на eslint 10 (`getFilename` API).
- `next build` кладёт sourcemap'ы с false-positive паттернами secret_scan — `.next/` обязан быть в .gitignore (добавлен 2026-08-25); сканер уважает .gitignore через `--exclude-standard`.
- Better Auth: тип ленивого инстанса выводить через `ReturnType<typeof createAuth>` (не `ReturnType<typeof betterAuth>` — generic-дефолт не совпадает с конкретными опциями).
...
- Last full `make verify` run by this agent: 2026-08-25 (PA-49 session 1) — PASS end-to-end после добавления `services/webapp` (npm ci по обновлённому lock; typecheck обоих workspace; collector TS-тесты + webapp vitest 11; pytest 51 passed / 1 skipped; contracts, migrations, provenance, architecture, boundary, secrets, vps, business-signal). Отдельно: webapp lint 0 ошибок; `next build` зелёный; smoke standalone (`/`→307→`/brief`, unreleased-плашка, `/styleguide`, `/login` 200).

## Exact next action

**PA-49 сессия 2 (деплой webapp).** От Mike: (1) имя домена + A-запись `app.<домен>` → 135.106.186.210; (2) явное подтверждение открытия 80/443 в firewall Selectel; (3) решение по push ветки `feat/pa-49-webapp-skeleton` (PR или прямой merge). Затем на VPS: схема `webapp_auth` + роли `webapp_auth_writer`/`webapp_readonly` (additive, вне M1-ледаря db/), секреты в `/etc/proxima-ai/secrets/` + path-only `.env`, сборка образа, `docker compose -f compose.yaml -f webapp.compose.yaml up -d webapp caddy`, первый пользователь Mike, замена cookie-presence на `auth.api.getSession` в (app)-layout, приёмка критериев 2-3. После PA-49: PA-50 (живой Brief на fixtures; контракт сигнала синхронизировать с PMM-29 bot lane). Living plan: `docs/exec-plans/active/pa-49-webapp-skeleton.md`.
...
Track A/C хвосты не изменились: COLLECTOR-WB-BRANCHES closure подтвердить с Mike (epic PA-36); PA-41 (W1 verbatim-import после аудита PA-39) — следующий на Треке A, не ждёт PMM-29 и не зависит от PA-49; пилотный XLSX (02-02) и компоненты PMM UI — по-прежнему у Mike.
...
Last updated: 2026-08-25 (PA-49 session 1)
