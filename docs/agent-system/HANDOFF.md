...
- **Story 1.3** → PR **#48**, ветка `feat/m01-story-1.3`. Ревью нашло два CRITICAL (тихий 0-row UPDATE в `RunLedger`; GUC арендатора переживал прогон на `pg.Pool`) и один MAJOR (гранты расходились с AD-11). Замечания ушли фикс-раундом в живой conversation `07f644d6` — воркер закрыл их коммитом `06c4b96`, история не переписана. Первая боевая обкатка фикс-раунда через мост.
...
**Открыто и требует единицы работы:** `*.db.test.ts` гоняются только через `pg_local_roundtrip.sh` с PG16 `initdb`; `.github/workflows/verify.yml` PostgreSQL не ставит, поэтому центральный тест story 1.3 зелёный только на маке Mike. Это незапушенный CI-коммит `d1a47bb` из блокера 31.08.
