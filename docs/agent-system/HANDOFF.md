...
Sprint 0 progress: PMM-30 done (DEC-006, PR #9); PMM-2 spike merged (PR #10: ADR-0001 + DEC-007, LE-1 pilot entity filled, stays Draft until accountant Q1-Q4 + remaining entities); PMM-9 + PMM-10 done (PR #12: `docs/governance/assumptions-register.md` 11 entries, `risk-register.md` 13 risks, both Jira Done). **PMM-29 = bot lane (claimed, do not duplicate).** **PMM-11: draft exit-criteria готовы 2026-08-27** — `docs/exec-plans/active/w1-slice-exit-criteria.md` (Draft, коммит `a8b8950`, ветка `pmm-11-exit-w1`) + Jira-комментарий-черновик 10367; awaiting Mike «утверждено» в Jira, затем прогон 2 (Approved-фиксация, описание PMM-11 + чек-лист, статус «В работе», ссылки PMM-1/3, risk-register запись о внешних входах, TASKS/HANDOFF хвост). Решения грилля: периметр = весь init-m2; V1 «результат» = действие + уведомление Богатовой (expected-vs-actual не блокирует); оба ADR только Accepted; реестры ≤30 дней + полнота + просмотр Mike; reviewer eval = adversarial пойман + 0 FP; velocity = задачи/календарь. Remaining sprint-0 candidates for the Proxima agent: PMM-7 (charter-pointer), PMM-8 (glossary + KPI tree), PMM-12 (DoD checklist) — pick PMM-7 next unless Mike reorders. PMM-31 needs explicit Mike approval (VPS operations). PMM-2 closes only at ADR Accepted.
