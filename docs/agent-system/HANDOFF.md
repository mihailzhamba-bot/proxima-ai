...
- 2026-08-27: **PMM-8 glossary + KPI tree implemented** (explicit Mike reprioritization): `docs/governance/m2-glossary-kpi.md` fixes 14 M2 terms and all 7 PV §8 metrics. Mike owns every metric; numeric thresholds remain `UNKNOWN` until the named baseline. `contribution` stays `UNKNOWN` pending ADR-0001/FIN-001; revenue-based value is separate. Product decisions: time-to-detect starts at source-fact availability; recall/FP use an AM-labelled audit set; every closed signal has an outcome; a monitored cabinet requires 7 consecutive successful scheduled cycles; paying client = signed agreement. Structural checks, `scripts/agent/verify` and full `make verify` PASS; Jira remains Backlog because this checkout has read-only Jira MCP access.
...
Sprint 0 progress: PMM-30 done (DEC-006, PR #9); PMM-2 spike merged (PR #10: ADR-0001 + DEC-007, LE-1 pilot entity filled, stays Draft until accountant Q1-Q4 + remaining entities); PMM-8 implemented 2026-08-27 by explicit Mike reorder; PMM-9 + PMM-10 done (PR #12: `docs/governance/assumptions-register.md` 11 entries, `risk-register.md` 13 risks, both Jira Done). **PMM-29 = bot lane (claimed, do not duplicate).** Remaining sprint-0 candidates for the Proxima agent: PMM-7 (charter-pointer), PMM-11 (exit criteria), PMM-12 (DoD checklist). PMM-31 needs explicit Mike approval (VPS operations). PMM-2 closes only at ADR Accepted.
