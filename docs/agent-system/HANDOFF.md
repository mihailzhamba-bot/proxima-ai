...
PA-49 (web-кабинет v0, подзадача 1/5 PA-38), сессия 1 завершена 2026-08-25 на ветке `feat/pa-49-webapp-skeleton` (не запушена): каркас `services/webapp` (Next.js 16.3 standalone + Better Auth 1.7 lazy + Tailwind 4 + Drizzle, две темы, GYR-токены, ₽-форматтеры, шелл с unreleased-плашкой DEC-006, скелеты brief/inbox/dashboard/admin + styleguide, vitest 11 тестов), деплой-контур (Dockerfile, `infra/webapp.compose.yaml` overlay + Caddyfile; основной compose.yaml не тронут), Makefile-хуки, `.gitignore` + `**/.next/`. `make verify` PASS целиком; smoke standalone-сервера зелёный. Living plan: `docs/exec-plans/active/pa-49-webapp-skeleton.md`. Сессия 2 = деплой на VPS: нужны от Mike домен + A-запись app.<домен> → 135.106.186.210 и подтверждение открытия 80/443; на VPS создать схему webapp_auth + роли webapp_auth_writer/webapp_readonly (additive, вне M1-ледаря), собрать образ, поднять overlay compose, завести первого пользователя, заменить cookie-presence на auth.api.getSession. Затем PA-50 (живой Daily Brief на fixtures; контракт сигнала брать у PMM-29 bot lane). Jira: PA-49 In Progress; PA-50..54 созданы со связями; PA-55 (DEC-запись про публичный URL) To Do.
