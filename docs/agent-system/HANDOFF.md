...
Sprint 0 progress: PMM-30 done (DEC-006, PR #9); PMM-2 spike merged (PR #10: ADR-0001 + DEC-007, LE-1 pilot entity filled, stays Draft until accountant Q1-Q4 + remaining entities); PMM-9 + PMM-10 done (PR #12: `docs/governance/assumptions-register.md` 11 entries, `risk-register.md` 13 risks + R-14, both Jira Done). **PMM-29 = bot lane (claimed, do not duplicate).** **PMM-11 approved 2026-08-27 (Mike, чат «Делай»)**: exit-критерии W1-среза утверждены — `docs/exec-plans/active/w1-slice-exit-criteria.md` (Approved; слепая приёмка 12/12; codex cross-model: раунд 1 — 1 blocker/5 warnings, все закрыты, re-review 0/0, `596f07c`); PR #22 (branch `pmm-11-exit-w1`); Jira-фиксация — комментарий с цитатой утверждения, описание PMM-11 = утверждённые критерии + чек-лист, статус «В работе», R-14 в risk-register (закрытие среза зависит от внешних входов: Q1-Q4, юрлица, вердикт Build vs Buy). Решения грилля: периметр = весь init-m2; V1 «результат» = действие AM вручную + уведомление Богатовой (expected-vs-actual не блокирует, доказательства шагов 4-6 обязательны); оба ADR только Accepted; реестры ≤30 дней + полнота по JQL-эталону + просмотр Mike; reviewer eval = adversarial пойман + 0 false BLOCK на полном датасете PMM-33; velocity = задачи/календарь (23 FULL-тикета, границы по changelog). Remaining sprint-0 candidates for the Proxima agent: PMM-7 (charter-pointer), PMM-8 (glossary + KPI tree), PMM-12 (DoD checklist) — pick PMM-7 next unless Mike reorders. PMM-31 needs explicit Mike approval (VPS operations). PMM-2 closes only at ADR Accepted.
