...
**Календарь:** 22.09 деплой 1.14 по слову «деплой» → 23-25.09 три утра SUCCEEDED → 26.09 чекпоинт → 27-28.09 подготовка домена (секреты `BETTER_AUTH_*`, прогон Caddy+auth на стенде репетиции, имя домена и A-запись от Mike) → 29.09 деплой 2.6 (postgres-витрина, воронка, снос ручного контейнера на 3000, Caddy+домен+auth, открытие 80/443) → 30.09 гейт M-03 (по ledger 3+ утр плюс факт живого экрана; оговорка в журнале релиза - `brief_current` в БД с 23.09, на экране с 29.09).

**Ждёт Mike:** слово «деплой» 22.09 и 29.09; пинг Владиславу по 6.1 (сегодня); имя домена + A-запись → 135.106.186.210 (до 28.09); ротация analytics-токена PA-13 в ЛК WB (read-only, до 2.6 - без неё воронка не включается); проверка копии артефактов бэкфилла в S3 (B6).
