...
Sprint 0 progress: PMM-30 done (DEC-006, PR #9); PMM-2 spike merged (PR #10: ADR-0001 + DEC-007, LE-1 pilot entity filled, stays Draft until accountant Q1-Q4 + remaining entities); PMM-9 + PMM-10 done (PR #12: `docs/governance/assumptions-register.md` 11 entries, `risk-register.md` 13 risks, both Jira Done). **PMM-12 done 2026-08-27 (DoD-чеклист: `docs/governance/dod-checklist.md` + verify guardrail; ретро-аудиты PMM-30/9/10 в Jira).** **PMM-29 = bot lane (claimed, do not duplicate).** Remaining sprint-0 candidates for the Proxima agent: PMM-7 (charter-pointer), PMM-8 (glossary + KPI tree), PMM-11 (exit criteria; fixations landed, Mike approved in-repo, Jira still «В работе») — pick PMM-7 next unless Mike reorders. PMM-31 needs explicit Mike approval (VPS operations). PMM-2 closes only at ADR Accepted.
