...
- 22.09 - деплой 1.14 **выполнен** (тег `v2026.09.22-3`, журнал `2026-09-22-m01.md`, инцидент WB_SCHEMA_DRIFT закрыт PR #163).
...
- 22.09 - аудит harper: `make verify` PASS с pg-roundtrip на `08043bb`, карта машины и бэклог - `docs/state/HOST-harper.md`; ветка `docs/d42-grill-plan` (D42 + аудит) запушена, PR/merge - за Mike. Вечер: §3b runbook 2.6 + публичный контур исполнены (`ca8a67e`: auth-роль/схема в provision - проверено живым PG16, боевой overlay, vps-контракт `caddy_https`, `Caddyfile.rehearsal`), полный verify PASS; Codex-аудит 13:06 нашёл P1-P6 - починка в этой же ветке (бриф: `.autopilot/2026-09-22-public-web-contour/`).

### Дневной прогон 09.09.2026: M1-M5 (D37)

### Деплой 1.14 выполнен 22.09.2026; приёмка трёх утр 23-25.09 - active

Деплой M-01 проведён 22.09 по слову «деплой» Mike (~09:45 UTC, чат автопилот-сессии morning-brief-live). Полный журнал - `docs/operations/releases/2026-09-22-m01.md` + `.autopilot/2026-09-21-morning-brief-live--wip/deploy-2026-09-22.md`.

- PR #162 (runbook D01-фикс webapp postgres-режима + артефакты рана) и PR #163 (`fix(collector)`: warehouse-only переименования WB больше не дрейф; append-only сохранён) - в `main`; теги `v2026.09.22-2` (`b68c2f6`, начало) и `v2026.09.22-3` (`36b6dbf`, контент после фикса); baseline отката `v2026.09.0-baseline` (`fd95fcb`).
- Сервер claudette: схема `18|18` (48 объектов, 58 политик), бэкфилл SUCCEEDED (13 386/10 675), живой collect SUCCEEDED, norm+brief SUCCEEDED, гейт §4 пройден (W10 649|700860.50; дни 24-26.08 точь-в-точь; 27-30.08 same=t), таймеры enabled (утро 05:30 МСК), алерт проверен (exit 0), webapp в postgres-режиме на `127.0.0.1:3000`.
- Инцидент 10:17 UTC: WB ретроактивно переименовал склады в закрытых записях (3 заказа, только `warehouseName`) → WB_SCHEMA_DRIFT, деплой остановлен; диагноз по CAS-артефактам, решение Mike - точечный фикс (PR #163), попытка 2 SUCCEEDED. FAILED-ран `9a7550f1` оставлен в леджере.
- **Дальше:** приёмка трёх утр 23-25.09 (тикет 04, Mike сверяет сам) - она же разморозка continuous Loop (B1 аудита mckenzie). Этап 3 - хвосты Loop (единый статус-снимок B3, paperclip=PLANNED, уборка PAT из remote URL, отчёт аудита в `/srv/loop/operations/audit-20260922/`). Этап 4 - Telegram-канал hermes via VPN (endpoint у Mike, данные к старту; api.telegram.org заблокирован провайдером с mckenzie точечно - github/google ходят).
- Follow-ups: WORKS-TODAY батарея целиком вручную (эквивалент покрыт CI verify PR #163 + гейтом §4); DECISIONS.md - решение по фикс-подходу задокументировано в журнале релиза и PR #163, номер D - при сверке нумерации (ветка d42 параллельной сессии).

### Дневной прогон 09.09.2026: M1-M5 (D37) - superseded деплеем 22.09
