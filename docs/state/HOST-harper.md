...
LOOP-очередь: последний батч `day-20260916-readiness-r3` завершён 20.09 14:37 (`completed` → `ready_pr` → **PR #145**, base `d5334f2`, head `a6fa928`, ревью glm-5.3-flash/z.ai); состояние `await_new_admitted_batch_manifest`, `automatic_job_admission: false` - легальный простой, ждёт операторского манифеста. Статусы пилотных PR LOOP (поправка 22.09 вечером по API-проверке Codex-аудита `docs/audits/2026-09-22-pr-and-merge-review.md`; мой метод ancestry ошибался - при squash/rebase-мёржах голова PR не попадает в целевую ветку): **#145 и #147 - смержены**; #154 (draft) - не самостоятельный второй merge, интеграционный кандидат D40 - **#160** (`00450c0`, HOLD: 9 modify/delete конфликтов с main, перенос ops-документов не доказан, cutover - отдельное решение Mike); из аудита же - LOOP-блокеры #161/#140 (container bind, childless stop) до отдельного решения не мержить.
...
**Пробел D38-D40 (реконструкция 22.09):** в `DECISIONS.md` обеих линий нумерация идёт D37 → D41. **D38 и D39 - ноль следов во всём дереве git** (pickaxe `-S` по всем рефам; единственные «попадания» - base64-хеши в `package-lock.json`), т.е. записи не существовало или она жила вне git (чат, LOOP-control). **D40 реконструирован из коммитов** (`3ad1eed`, `529085f`, `70dd3b2`, `186cb7c`, `00450c0`): расщепление репо на публичный код и приватный ops repo - sanitized base `529085f` «public-main+workflow-verbatim», docs/_bmad-output уходят в ops repo («migration pending»), tenant → `pilot-tenant`, hygiene gate в verify, docs-зависимые чеки скипаются без ops composite, cutover только по явному «деплой» Mike; merge `00450c0` приготовлен и запушен (`chore/d40-land`), PR #154 открыт, post-`6912a92` доки (#158/#159) остаются в main до миграции. Запись D40 в `DECISIONS.md` отсутствует - дописать канон может только Mike; вопрос: существуют ли D38/D39 и где должен жить ops repo.
