...
GRANT SELECT ON collector_runs TO proxima_job_norm;
GRANT SELECT ON collector_runs TO proxima_webapp_readonly;
GRANT SELECT ON collector_run_inputs TO proxima_webapp_readonly;
GRANT SELECT ON wb_raw_artifacts TO proxima_webapp_readonly;
...
CREATE POLICY tenant_isolation_norm ON collector_runs FOR SELECT TO proxima_job_norm USING (tenant_id = current_setting('proxima.tenant_id', true));
...
CREATE POLICY tenant_isolation_webapp ON collector_run_inputs FOR SELECT TO proxima_webapp_readonly USING (tenant_id = current_setting('proxima.tenant_id', true));
...
CREATE POLICY tenant_isolation_webapp ON wb_raw_artifacts FOR SELECT TO proxima_webapp_readonly USING (tenant_id = current_setting('proxima.tenant_id', true));
...
VALUES (11, 'run_ledger', '4a7f0f508b2a6cbca54e38af85599f23eb12451f2a8279a1d2879bc895859dbb');
COMMIT;
