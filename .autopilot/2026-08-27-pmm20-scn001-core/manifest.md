...
|----|---------------------|--------|-----------|-----|
| R01 | «baseline 7/14/28 с сезонностью дня недели» - мультипликативный индекс s_w; Expected(D,W) = MA(W) × s_w(D); триггер Expected(28) + корроборация 7/14; сигнал несёт все три дельты | done | 1569a6e (T01, 2 раунда ревью) | T01 |
| R02 | «Декомпозиция отклонения: U×CVR×AOV, вклад каждого множителя» - Шепли, вклады суммируются в дельту ₽ точно | done | 1569a6e (T01, 2 раунда ревью) | T01 |
| R03 | Метрика = заказы: U=openCard, CVR=orders/openCard, AOV=ordersSumRub/orders; выкупы контекстом в payload | done | 1569a6e (T01, 2 раунда ревью) | T01 |
| R04 | «Порог: глобальный по умолчанию» - дефолт 20% против Expected(28) в конфиге + interface stub под Client Passport (safe default, PMM-21 rollback-режим) | done | 1569a6e (T01, 2 раунда ревью) | T01 |
| R05 | «₽-фильтр значимости (сигналов ≤10/день на кабинет)» - revenue_delta_orders = Expected(D) − Actual(D), floor 3 000 ₽/день, топ-≤10 по убыванию | done | 1569a6e (T01, 2 раунда ревью) | T01 |
...
| R13 | Ядро в `services/control-plane/src/proxima_control_plane/detectors/scn001/` (Python 3.14, uv), деньги в Decimal | done | 1569a6e (T01, 2 раунда ревью) | T01 |
| R14 | Тесты на явно помеченных инжектированных кейсах: FP-кейс выходных (нет сигнала), кейс падения (сигнал + вклады), детерминизм, дедуп, ₽-фильтр top-10 | done | 1569a6e (T01, 2 раунда ревью) | T01 |
| R15 | «Детерминированный расчёт (LLM запрещён в числах)» | done | 1569a6e (T01, 2 раунда ревью) | T01 |
| R16 | Read-only smoke против staging при поднятом туннеле: одна реальная строка payload, сверка реестра маппинга; туннель не поднят → UNKNOWN зафиксировать | done | T02 (loader/smoke, ревью 0 blocking) | T02 |
| R17 | Jira: PMM-20 → «В работе», комментарий с решениями грилля | done | статус «В работе» + комментарий 10376 (2026-08-28) | финал |
| R18 | Verify-гейт: `make verify` + чеклист PMM-12 (`docs/governance/dod-checklist.md`) + independent reviewer | done | T02 (loader/smoke, ревью 0 blocking) | T02 |
| R19i | Zero-guards: baseline=0, openCard=0 при orders>0, отрицательные значения - явные fail-closed кейсы, не crash/NaN | done | 1569a6e (T01, 2 раунда ревью) | T01 |
| R20i | Всё помечено `unreleased` (DEC-006: staging-данные пилота вне release-контура) | done | 1569a6e (T01, 2 раунда ревью) | T01 |
