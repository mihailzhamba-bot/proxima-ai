...
function TaskCard({ task, role }: { task: TaskItem; role: "owner" | "employee" }) {
  ...
  return <article className="rounded-md border border-border bg-card p-5">
    ...
    <p className="mt-3 text-sm">Ожидаем: {task.expected_outcome}</p><div className="mt-2 space-y-2 text-sm">{task.expected_metrics.map((metric, i) => <div key={i}><p>Цель: {metric.name === "orders" ? "заказы" : metric.name === "revenue" ? "выручка" : metric.name} <span className="font-mono">{formatTarget(metric.value, metric.unit)}</span></p><p className="break-words text-xs text-muted-foreground">Источник цели: {metric.source_ref}</p></div>)}</div><p className="mt-2 text-sm text-muted-foreground">Наблюдение после выполнения: <span className="font-mono">{task.horizon_days}</span> дн.</p>
    ...
    {task.observation?.snapshot && <details className="mt-3 text-sm"><summary className="cursor-pointer py-2">Показатели и источники проверки</summary><p className="font-mono">День: {task.observation.snapshot.evaluation_day}</p>{task.observation.snapshot.measurements.map((m, i) => <div key={i} className="mt-3 space-y-1"><p>{m.name === "orders" ? "Заказы" : "Выручка"}: <span className="font-mono">{formatTarget(m.actual, m.unit)}</span>; цель <span className="font-mono">{formatTarget(m.expected, m.unit)}</span></p>{m.source_refs.map(ref => <p className="break-all text-xs text-muted-foreground" key={ref}>Источник: {ref}</p>)}</div>)}</details>}
