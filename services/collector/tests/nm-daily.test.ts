...
test('nm-daily: per_nm_sums_vs_cabinet result shape from the check rows', async () => {
  ...
  const pass = await checkPerNmSumsVsCabinet(fake(clean), { tenantId: 'pilot-tenant', runId: RUN });
  assert.deepEqual(pass, { check: 'per_nm_sums_vs_cabinet', status: 'PASS', days_checked: 2, mismatches: [] });
  assert.deepEqual(queries[0]?.values, ['pilot-tenant', RUN]);
  ...
  const mismatch = await checkPerNmSumsVsCabinet(fake(broken), { tenantId: 'pilot-tenant' });
  ...
  assert.deepEqual(queries[1]?.values, ['pilot-tenant']);
  ...
  await assert.rejects(() => checkPerNmSumsVsCabinet(fake([{ calendar_day: '2026-08-28', column_name: 'orders', cabinet: '1', per_nm_sum: '1', mismatch: false }]), { tenantId: 'pilot-tenant' }), RangeError);
});
