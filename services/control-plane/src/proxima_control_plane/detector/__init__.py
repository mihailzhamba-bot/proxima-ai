...
Порог - значение конфигурации control-plane (`threshold.py` / `threshold.toml`,
решение 6а, Story 4.2): до Story 4.4 он `null` и не применяется - кандидат - каждое
отклонение ниже нормы; порядок - по деньгам под риском (CAP-7), от порога не зависит.
"""
