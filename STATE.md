...
**Обновлено:** 01.09.2026 (ночная смена 31.08→01.09): сданы конвейером Stories 1.0, 1.1, 1.2, 1.10, 1.12, 2.1 + ADR 0007 (PA-21) - PR #36-#41 открыты, каждый с враждебным ревью и зелёным verify на маке; Jira-sync выполнен (8 эпиков лестницы, 22 задачи, 36 вердиктов бэклога - `docs/state/JIRA-SYNC-2026-08-31.md`); Story 1.9 сдана последней (PR #42; HIGH ревью - маска `**/fixtures/` ломала образ webapp - закрыт, образ проверен живьём в песочнице). **Ворота 3:** пройдены 31.08 (D23). Инциденты ночи: systemd-лимиты OpenHands (TasksMax 512, Mem 8G) уронили 6 воркеров - подняты до 4096/12G, все воркеры восстановлены продолжениями без потерь git-состояния. Агенты: Fedor/Codex (gpt-5.6-terra) и GLM пополам по решению Mike.
...
**Story 1.1, попытка 1 (31.08, conversation `d88d4273…`) - брак, причина = среда.** Приватный репо не клонируется с GitHub без креденшалов (404) - воркер молча взял stale-кэш main @ `db56429` без `feat/m01-step0` и без docs-ветки, т.е. работал без контракта, без API-FACTS и без кода 1.0. Результат (6 коммитов, свой verify_wb_client PASS): выдуманные URL (включая снятый v2 `nm-report/detail`), собственный envelope-формат фикстур с base64 вместо AD-4, второй анонимайзер без соли, фикстуры с выдуманными данными. Ветка сохранена локально на маке как `story-1.1-attempt1` (не пушить), бандл в scratchpad. Ядро (бюджеты, 429, sink, mskDay, тестовые скаффолды) выглядело здраво - брак именно во входах.

**Story 1.1, попытка 2 (31.08, запущена):** conversation `eeff5358-069a-4ac3-8398-a2ae7eeeecbe`, workspace `/srv/openhands/persistence/workspace/project/eeff5358069a4ac38398a2ae7eeeecbe` спровижен вручную: клон из бандла (main + `feat/m01-step0` @ `0f307c4` + docs-ветка), ветка `feat/m01-story-1.1` от `0f307c4` (НЕ от локального `bfdb6bf` - чтобы rescue-push не упёрся в workflow-scope), справочные `docs/state/` + `_bmad-output/` положены untracked. Промт ужесточён: URL только из API-FACTS, формат фикстур AD-4 фиксирован, чужой анонимайзер не переписывать, «нет факта - вопрос, не выдумка». Push из сандбокса невозможен: забор результата бандлом (`sudo git -C <ws>/proxima-ai -c safe.directory='*' bundle create …` → scp → verify на маке → push). Scar: перед запуском единицы в OpenHands проверять, что база воркера и контрактные файлы реально в его workspace - «клонируй сам» не работает для приватного репо.

**Story 1.1 сдана конвейером (31.08): PR #37** (base `feat/m01-step0` - перетаргетится на main после мержа #36). Попытка 2 попала в контракт: URL из API-FACTS, настоящие фикстуры 1.0, AD-4 порядок «артефакт до статуса». Враждебное ревью: 0 CRITICAL, 3 MAJOR (нет CLI token-file флагов; гейт обходим - мёртвая extra-проверка/дубли URL/literal-only host-regex; RateBudget пробивается конкурентными reserve) - возвращены воркеру в тот же conversation, закрыты 4 коммитами (итого 9). Verify на маке: зелёный целиком, `pg-roundtrip: PASS`; атака-инъекция на гейт воспроизведена независимо - FAIL на инъекции, PASS после отката. sprint-status: `1-1: review`. Ветка попытки 1 - локально `story-1.1-attempt1` (не пушить, референс).

Дальше: Mike - PAT Workflows write (пуш `bfdb6bf`), мерж #35, squash-мерж #36 (после него перебазирую #37), приёмка #37 (`make verify` → `verify_wb_client: PASS`); затем тег + безопасный деплой-шаг AR11 п.1 по явному «деплой», Story 1.2 в OpenHands по той же схеме (workspace провижу из бандла).
