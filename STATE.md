...
**Обновлено:** 03.09.2026. Канон решений - `DECISIONS.md` (D1-D26), этот файл - память между сессиями, не реестр решений.

**Где продукт.** 9 из 22 единиц сентября сделаны (`_bmad-output/implementation-artifacts/sprint-status.yaml` на `origin/main`: 1.0-1.3, 1.9, 1.10, 1.12, 2.1, 2.2). Очередь после правила сжатия (D25, решение 2а): `1.4 → 1.5 → 1.6 → 1.7 → 1.13 → 1.14 → 2.3 → 2.4 → (2.5 + 1.11) → 2.6`, параллельно 3.1 в работу не позже 08.09. Даты релизов: 1.14 не позже 20.09, 2.6 не позже 23.09 - **ждут подтверждения Mike календарём**, иначе семь утр гейта 24-30.09 не помещаются.

**Что решено 02-03.09** (`DECISIONS.md` D25, D26): правило сжатия вместо ветки сброса; SM-7 = N недель воронки от даты включения (8 недель недостижимы); change request к AD-6 (воронка отдельным юнитом после сводки); бэкфилл 1.5 с реальными артефактами в main до 08.09 одним тегом с 1.14; предварительный порог тревоги 30 % на падение; Policy Layer отложен в CM-20; ротация analytics-токена как инцидент; роль Владислава - независимый теневой пересчёт всей цепочки с правом блокировать релиз (Epic 6, `docs/agent-system/roles/analyst-vladislav.md`).

**Три расхождения канона - агент, читающий SPEC и спайн буквально, построит не то** (правки заведены как OQ-16 и задача PA-64, до них расхождение действует):

1. `SPEC.md` CAP-6 всё ещё требует «8 недель воронки к 27.10» - снято решением 3а (D25).
2. `ARCHITECTURE-SPINE.md` AD-6 всё ещё описывает цепочку `collect → funnel_v3 → norm → brief` одним юнитом - заменено решением 4а; **блокирует мерж Story 3.1**.
3. Спайн Deferred всё ещё пишет «порог тревоги после первых живых сводок» - решением 6а принят предварительный порог 30 %.

**Не отражено нигде в архитектуре и реестрах:** подсистема кондуктора (Hermes) - пять коммитов 01.09, systemd-юниты `proxima-conductor*`, `docs/agent-system/ORCHESTRATOR.md`, `infra/hermes/`. Ни записи решения, ни места в спайне.

**Состояние машины.** Боевой чекаут `/srv/proxima-ai/repo` - detached на `fd95fcb` от 25.08, без `remote`, 210 коммитов позади main, утренних таймеров нет, миграции 001-006 против 010 в main. Первый релиз - это оживление сервера, а не «включить таймер» (порядок - в AC Story 1.13). Рабочее дерево `sunfish` отстаёт от `main` на 22 коммита: контракты `norm`/`brief`/`cabinet-daily` и миграция 011 есть в `main`, но не видны локально.

**Ждёт Mike:** две даты деплоя; конвенции расчёта (OQ-18: медиана при чётном n, округление денег и процентов, нумерация недель) до первого пересчёта; approve брифа Jira `docs/state/JIRA-SYNC-BRIEF-2026-09-03.md`; доступы Владиславу и решение по LOGIN-роли для чтения базы; команда на пуш ветки `docs/prd-ladder-2026-09-02` (три коммита, 142 файла, локально).

**История ниже** (записи 30.08-01.09) сохранена как есть; при расхождении с этим блоком верен этот блок.

<!-- предыдущая шапка, 01.09 -->
