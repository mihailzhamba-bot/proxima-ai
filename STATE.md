...
**Обновлено:** 30.08.2026, конец Сессии 1b (окно A).
**Следующая сессия:** 2 - после того, как Mike ответит на 6 вопросов в `docs/state/GATE-1.md` и бэклог из окна B ляжет в `BACKLOG-REVIEW.md`.
...
**Окно B - в работе:** `docs/state/BACKLOG-REVIEW.md` через Jira MCP (PA + PMM, только чтение). Коммитит только этот файл, `git pull --rebase` перед push. В Jira не пишет.
...
2. Проверить, что `BACKLOG-REVIEW.md` есть в ветке (окно B); если нет - сделать его первым (Jira MCP read-only).
