...
**Обновлено:** 30.08.2026, конец Сессии 3 (D22). **Следующая сессия:** 4 - нарезка (`bmad-create-epics-and-stories`) и ворота готовности (`bmad-sprint-planning`) → Ворота 3. Перед стартом Mike подтверждает 5 допущений спайна (D22).
...
## С чего начинать Сессию 4

1. Прочитать этот файл, `DECISIONS.md` (D22 - 5 допущений спайна: подтверждены ли Mike), `SPEC.md`, `ARCHITECTURE-SPINE.md` (18 AD). Допущения не подтверждены - спросить первым раундом.
2. Этап 8: `bmad-create-epics-and-stories` по SPEC + спайну. Правила поверх стандартных (промт): единица работы = один сеанс OpenHands + один PR + один результат, проверяемый Mike одним действием; в единицу кладётся только принятое решение (ссылки на AD/D); первой идёт маленькая единица, обкатывающая конвейер целиком - U2 «WB-клиент + FixtureTransport + verify_wb_client + проверка `flag=0` и уникальности `srid` (2 вызова)»; вероятная нарезка U1-U10 - в `reviews/review-adversarial-v2.md` (шапка задачи B); что из старого бэклога сворачивается - явно, по `BACKLOG-REVIEW.md`. Лестница M-00..M-05 - вход и гипотеза; разошлось - объяснить Mike.
3. Этап 9: `bmad-sprint-planning` → PASS/CONCERNS/FAIL + `sprint-status.yaml`; FAIL/CONCERNS - дорабатывать. Плюс приёмочный тест: перечитать каждую единицу с чистого листа - возник вопрос, не готова.
4. Ворота 3: список единиц в порядке исполнения; Mike режет объём (сентябрь / за борт). Только после этого - Jira (D17): 6 эпиков по ступеням + задача на единицу.
5. Обновить STATE.md, push, PR.
