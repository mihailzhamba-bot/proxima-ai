...
```bash
scripts/agent/now facade opencode --stdin
```
...
Close JSON is a request, never authority. Run `scripts/agent/now close --client opencode --evidence <close.json>` to validate the exact schema and obtain the `now.close.v1` side-effect plan. Execute it only through opencode's explicitly approved Jira/GitHub/Orca adapters, independently read back durable reviewer, Mike decision-gate and ops-sync receipts, then provide them through injected close ports. The local CLI stays `BLOCKED_EXTERNAL` and cannot claim `DONE` itself.
